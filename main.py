...
# attempt to get S&P500 (^GSPC) data from yahoo finance
sp500_df = web.DataReader('^GSPC', 'yahoo', start, end)
# sp500_df = web.get_data_yahoo('^GSPC', start, end)
...
counter = 0  # in order to not run through all 500 companies
...
best_performers = best_performers[
    best_performers['Score'] >= best_performers['Score'].quantile(0.7)]  # pick the top 30%
...
for ticker in best_performers['Ticker']:
    try:
        ...
        for ma in moving_averages:
            df['SMA_' + str(ma)] = round(df['Adj Close'].rolling(window=ma).mean(), 2)  # aggregate
        ...
        low_52_week = round(min(df['Low'][-(52 * 5):]),
                            2)  # 5 days a week, 52 weeks (to avoid saturdays and sundays) (not avoids holidays)
